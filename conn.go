...
// DialI2P makes a (un-secure) TELNET client connection to the system's 'loopback address'
// (also known as "localhost" or 127.0.0.1).
//
// If a secure connection is desired, use `DialTLS` instead.
func DialI2P() (*Conn, error) {
	return DialTo("")
}

// DialToI2P makes a (un-secure) TELNET client connection to the the address specified by
// 'addr'.
//
// If a secure connection is desired, use `DialToTLS` instead.
func DialToI2P(addr string) (*Conn, error) {
